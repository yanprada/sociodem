...
from src.databases.bronze.ibge.config import CONTRACTS_BRONZE as CONTRACT_CENSO_BRONZE
from src.databases.bronze.pois.config import CONTRACTS_BRONZE as CONTRACT_POIS_BRONZE
...
class Loader:
    ...
    def get_places():
        ...
        path = get_db_path(contract_dompp)
        conn = DBConnection("bronze")
        query = f"SELECT general_category, hex_col FROM {path}"
